...
# Loop over the first two identities
for (i in 1:2) { # Adjust this to 3:4 for the next two identities and so on
  # Define the current identity to compare against the rest
  current_identity <- identities[i]
  
  # Define the rest of the identities
  other_identities <- identities[-i]
  
  # Construct the contrast dynamically
  contrast <- paste0("cond(stage_grouped = 'uninfected_adult', identity = '", current_identity, "') - (", 
                     paste0("cond(stage_grouped = 'uninfected_adult', identity = '", other_identities, "')", collapse = " + "), 
                     ")")
  
  # Perform the test_de function and subsequent operations
  res <- test_de(fit, contrast = eval(parse(text = contrast)),
                 max_lfc = 5,
                 verbose = TRUE) %>%
    dplyr::select(-c(df1, df2)) %>%
    dplyr::rename("Symbol" = "name") %>%
  ...
  file_name <- paste0("/vol/ExtraVol/ParasiteDEX/ParasiteDEX_PairwiseMarkerResults_", gsub("\\s+|\\+|\\-", "_", current_identity), ".xlsx")
  
  # Save the result to an Excel file
  write.xlsx(res, file_name, overwrite = TRUE)
  
  # Print a message indicating completion of the current identity
  message(paste("Completed and saved results for", current_identity, "to", file_name))
}
```
...
# Save the workbook
saveWorkbook(wb, "/vol/ExtraVol/ParasiteDEX/ParasiteDEX_PairwiseResults.xlsx", overwrite = TRUE)
```
...
write.xlsx(res_main_infection, "/vol/ExtraVol/ParasiteDEX/ParasiteDEX_MainInfectionResults.xlsx", overwrite = TRUE)
```
...
write.xlsx(res_interaction, "/vol/ExtraVol/ParasiteDEX/ParasiteDEX_InteractionResults.xlsx", overwrite = TRUE)
```
